...
def generate_launch_description():
    description_path = os.path.join(
        get_package_share_directory('ridgeback_description'), 'launch', 'description.launch.py')
    ridgeback_controls_path = os.path.join(
        get_package_share_directory('ridgeback_gazebo'), 'launch', 'start_ridgeback_controls.launch.py')
    ...
    declare_joy_cmd = DeclareLaunchArgument(
        ...
        description='system using joystick')
    ...
    description_cmd = IncludeLaunchDescription(
        ...
        launch_arguments={'config': config, 'physical_robot': 'false',
    ...
    # might need to add a yaw command to the above line #

    delayed_ridgeback_controls_cmd = TimerAction(
        period=20.0,
        actions=[
    ...
    ld.add_action(delayed_ridgeback_controls_cmd)
